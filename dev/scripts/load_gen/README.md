...
如果服务分布在不同机器，请将命令中的 `127.0.0.1` 替换为对应机器的可访问 IP。
...
| SGLang 解码实例 | 30001–30003 | 预填充/解码分离的 D 实例 |
...
```text
dev/scripts/launch_server_p.sh
dev/scripts/launch_server_d.sh
dev/scripts/launch_pd_router.sh
```
...
nohup bash dev/scripts/launch_server_p.sh 0 30000 \
...
下面示例启动三个 D 实例。请根据实际 GPU 数量调整 GPU 编号和端口：

```bash
cd /sgl-workspace/sglang

nohup bash dev/scripts/launch_server_d.sh 1 30001 \
  > /tmp/load_gen_logs/sglang_decode_1.log 2>&1 &
echo $! > /tmp/load_gen_pids/sglang_decode_1.pid

nohup bash dev/scripts/launch_server_d.sh 2 30002 \
  > /tmp/load_gen_logs/sglang_decode_2.log 2>&1 &
echo $! > /tmp/load_gen_pids/sglang_decode_2.pid

nohup bash dev/scripts/launch_server_d.sh 3 30003 \
  > /tmp/load_gen_logs/sglang_decode_3.log 2>&1 &
echo $! > /tmp/load_gen_pids/sglang_decode_3.pid
```
...
如果只启动一个或两个 D 实例，需要同步修改 `dev/scripts/launch_pd_router.sh` 中的 `--decode` 地址列表。
...
nohup bash dev/scripts/launch_pd_router.sh \
