...
inline std::vector<int64_t> allocate_kv_splits(
  ...
  TORCH_CHECK(total_budget > 0, "decode_adaptive: total split budget must be > 0.");
  TORCH_CHECK(batches > 0, "decode_adaptive: batches must be > 0.");
  TORCH_CHECK(
      total_budget >= batches,
      "decode_adaptive: total split budget (",
      total_budget,
      ") must be >= batches (",
      batches,
  ...
  for (int64_t i = 0; i < batches; ++i) {
    TORCH_CHECK(seq_lens[i] > 0, "decode_adaptive: seq_len must be > 0.");
    sum_len += seq_lens[i];
  }
  TORCH_CHECK(sum_len > 0, "decode_adaptive: sum(seq_lens) must be > 0.");
  ...
  TORCH_CHECK(check_sum == total_budget, "decode_adaptive: internal split allocation sum mismatch.");
  return splits;
}
...
void decode_accumulate_kv_splits_adaptive(
  ...
  const int64_t l_stride0 = num_heads * attn_logits_splits * (head_size_v + 1);
  ...
  at::parallel_for(0, batches * num_heads, 0, [&](int64_t begin, int64_t end) {
    for (int64_t i = begin; i < end; ++i) {
      ...
      float* __restrict__ acc = attn_logits + bs * l_stride0 + head * l_stride1;
...
void decode_attention_mla_adaptive_kernel_impl(
...

  {
    ...
    decode_accumulate_kv_splits_adaptive(
        output, attn_logits, kv_splits_per_seq, batches, num_heads, head_size_v, attn_logits_splits);
  }
}
...
void decode_attention_cpu_adaptive(
    at::Tensor& query,
    at::Tensor& k_buffer,
    at::Tensor& v_buffer,
    at::Tensor& output,
    ...
    int64_t head_block_num) {
  RECORD_FUNCTION(
      "sgl-kernel::decode_attention_cpu_adaptive",
  ...
  TORCH_CHECK(head_block_num > 0, "decode_adaptive: expect head_block_num > 0, got ", head_block_num);
  ...
  TORCH_CHECK(num_heads_kv > 0, "decode_adaptive: expect num_heads_kv > 0, got ", num_heads_kv);
  TORCH_CHECK(
      num_heads % num_heads_kv == 0,
      "decode_adaptive: expect num_heads divisible by num_heads_kv, got ",
      num_heads,
      " and ",
      num_heads_kv);
  ...
  TORCH_CHECK(
      ...
      "decode_adaptive: expect req_to_token to be int32 or int64, got ",
      index_dtype);
  TORCH_CHECK(
      seq_lens.scalar_type() == at::kLong,
      "decode_adaptive: expect req_lens to be int64, got ",
      seq_lens.scalar_type());
  TORCH_CHECK(
      req_pool_indices.scalar_type() == at::kLong,
      "decode_adaptive: expect req_pool_indices to be int64, got ",
  ...
  TORCH_CHECK(num_threads > 0, "decode_adaptive: expect num_threads > 0, got ", num_threads);

  int64_t total_kv_splits_budget = 0;
  if (is_mla) {
    TORCH_CHECK(
        num_threads % head_block_num == 0,
        "decode_adaptive MLA: expect num_threads % head_block_num == 0, got num_threads=",
        num_threads,
        ", head_block_num=",
        head_block_num);
    total_kv_splits_budget = num_threads / head_block_num;
  } else {
    int64_t denom = head_block_num * num_heads_kv;
    TORCH_CHECK(
        num_threads % denom == 0,
        "decode_adaptive packed GQA: expect num_threads % (head_block_num*num_heads_kv) == 0, got num_threads=",
        num_threads,
        ", head_block_num=",
        head_block_num,
        ", num_heads_kv=",
  ...
  TORCH_CHECK(total_kv_splits_budget > 0, "decode_adaptive: derived total split budget must be > 0.");
  ...
  TORCH_CHECK(
      ...
      "decode_adaptive: attn_logits.shape[2] must be >= max per-seq split (",
  ...
  if (is_mla) {
    TORCH_CHECK(block_n == BLOCK_N_MLA, "decode_adaptive MLA: unsupported BLOCK_N ", block_n);
  }
      ...
      if (is_mla) {
        if (timer_on) {
          decode_timer::ScopedStageTimer<true> kernel_total_timer(
              ...
              /*tid=*/0);
          decode_attention_mla_adaptive_kernel_impl<scalar_t, index_t, BLOCK_N_MLA, true>(
          ...
          decode_attention_mla_adaptive_kernel_impl<scalar_t, index_t, BLOCK_N_MLA, false>(
              output.data_ptr<scalar_t>(),
              attn_logits.data_ptr<float>(),
              query.data_ptr<scalar_t>(),
              (const scalar_t*)k_buffer_data,
        ...
        if (timer_on) {
          decode_timer::ScopedStageTimer<true> kernel_total_timer(
              decode_timer::Stage::kKernelImplTotal,
              /*tid=*/0);
          decode_attention_grouped_packed_adaptive_kernel_impl<scalar_t, index_t, BLOCK_N_GQA_PACKED, true>(
              output.data_ptr<scalar_t>(),
              attn_logits.data_ptr<float>(),
              query.data_ptr<scalar_t>(),
              (const scalar_t*)k_buffer_data,
          ...
          decode_attention_grouped_packed_adaptive_kernel_impl<scalar_t, index_t, BLOCK_N_GQA_PACKED, false>(
              output.data_ptr<scalar_t>(),
              attn_logits.data_ptr<float>(),
              query.data_ptr<scalar_t>(),
              (const scalar_t*)k_buffer_data,
