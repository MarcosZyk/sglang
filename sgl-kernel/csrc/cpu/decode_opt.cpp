...
inline std::vector<int64_t> allocate_kv_splits(
    const int64_t* __restrict__ seq_lens,
    int64_t batches,
    int64_t total_budget) {
  TORCH_CHECK(total_budget > 0, "decode_opt: total split budget must be > 0.");
  TORCH_CHECK(batches > 0, "decode_opt: batches must be > 0.");
  TORCH_CHECK(
      total_budget >= batches,
      "decode_opt: total split budget (",
      total_budget,
      ") must be >= batches (",
      batches,
      ") so every sequence gets at least one split.");

  int64_t sum_len = 0;
  for (int64_t i = 0; i < batches; ++i) {
    TORCH_CHECK(seq_lens[i] > 0, "decode_opt: seq_len must be > 0.");
    sum_len += seq_lens[i];
  }
  TORCH_CHECK(sum_len > 0, "decode_opt: sum(seq_lens) must be > 0.");

  std::vector<int64_t> splits(batches, 1);
  int64_t remain_budget = total_budget - batches;
  if (remain_budget == 0) {
    return splits;
  }

  std::vector<double> frac(batches, 0.0);
  int64_t extra_assigned = 0;
  for (int64_t i = 0; i < batches; ++i) {
    double raw = static_cast<double>(remain_budget) * static_cast<double>(seq_lens[i]) / static_cast<double>(sum_len);
    int64_t base = static_cast<int64_t>(std::floor(raw));
    splits[i] += base;
    extra_assigned += base;
    frac[i] = raw - static_cast<double>(base);
  }

  int64_t leftovers = remain_budget - extra_assigned;
  std::vector<int64_t> order(batches);
  for (int64_t i = 0; i < batches; ++i) {
    order[i] = i;
  }
  ...
  for (int64_t i = 0; i < leftovers; ++i) {
    splits[order[i % batches]] += 1;
  }
  ...
  TORCH_CHECK(check_sum == total_budget, "decode_opt: internal split allocation sum mismatch.");
  return splits;
}

inline std::vector<int64_t> build_seq_task_prefix(
    const std::vector<int64_t>& kv_splits_per_seq,
    int64_t task_mul_per_seq) {
  const int64_t batches = static_cast<int64_t>(kv_splits_per_seq.size());
  std::vector<int64_t> prefix(batches + 1, 0);
  for (int64_t i = 0; i < batches; ++i) {
    prefix[i + 1] = prefix[i] + task_mul_per_seq * kv_splits_per_seq[i];
  }
  return prefix;
}

template <typename scalar_t>
void decode_accumulate_kv_splits_adaptive(
    scalar_t* __restrict__ output,
    float* __restrict__ attn_logits,
    const int64_t* __restrict__ kv_splits_per_seq,
    const int64_t* __restrict__ seq_split_prefix,
    int64_t batches,
    int64_t num_heads,
    int64_t head_size_v,
    int64_t attn_logits_splits) {
  using Vec = at::vec::Vectorized<float>;
  const int64_t l_stride1 = attn_logits_splits * (head_size_v + 1);
  const int64_t l_stride2 = head_size_v + 1;

  at::parallel_for(0, batches * num_heads, 0, [&](int64_t begin, int64_t end) {
    for (int64_t i = begin; i < end; ++i) {
      const int64_t bs = i / num_heads;
      const int64_t head = i % num_heads;
      const int64_t num_kv_splits = kv_splits_per_seq[bs];
      const int64_t split_base = seq_split_prefix[bs];

      float* __restrict__ acc = attn_logits + head * l_stride1 + split_base * l_stride2;
...
void decode_attention_mla_adaptive_kernel_impl(
    scalar_t* __restrict__ output,
    float* __restrict__ attn_logits,
    ...
    const int64_t* __restrict__ seq_lens,
    const int64_t* __restrict__ kv_splits_per_seq,
    const int64_t* __restrict__ seq_task_prefix,
    const int64_t* __restrict__ seq_split_prefix,
...

  {
    ...
    for (int64_t task = begin; task < end; ++task) {
      int64_t bs = std::upper_bound(seq_task_prefix + 1, seq_task_prefix + batches + 1, task) - (seq_task_prefix + 1);
      int64_t local = task - seq_task_prefix[bs];
      int64_t kv_splits = kv_splits_per_seq[bs];
      int64_t block_id = local / kv_splits;
      int64_t kv_id = local % kv_splits;
      const int64_t global_split_id = seq_split_prefix[bs] + kv_id;
      ...
      const int64_t split_size = div_up(seq_len_kv, kv_splits);
      ...
      float* __restrict__ v_prime = attn_logits + h_start * l_stride1 + global_split_id * l_stride2;
      ...
      for (int64_t n = kv_start; n < kv_end; n += BLOCK_N) {
        ...
        int64_t padded_n_size = div_up(int(n_size), TILE_K) * TILE_K;
        ...
        at::native::cpublas::brgemm(
            h_size, n_size, head_size, q_strideH, BLOCK_N, BLOCK_N, false, q_ptr, Btmp0, s_i);
        ...
        at::native::cpublas::brgemm(
            h_size, head_size_v, padded_n_size, BLOCK_N, head_size_v, l_stride1, true, s_delta2, Btmp1, v_prime);
      }
  ...
  decode_accumulate_kv_splits_adaptive(
      output, attn_logits, kv_splits_per_seq, seq_split_prefix, batches, num_heads, head_size_v, attn_logits_splits);
}

template <typename scalar_t, typename index_t, int64_t BLOCK_N>
void decode_attention_grouped_packed_adaptive_kernel_impl(
    scalar_t* __restrict__ output,
    float* __restrict__ attn_logits,
    const scalar_t* __restrict__ query,
    const scalar_t* __restrict__ k_buffer,
...

  {
    ...
    for (int64_t task = begin; task < end; ++task) {
      ...
      int64_t split_domain = tasks_per_split * kv_splits;
      int64_t kv_id = local % kv_splits;
      const int64_t global_split_id = seq_split_prefix[bs] + kv_id;
      int64_t tmp = (local % split_domain) / kv_splits;
      int64_t head_kv_id = tmp / num_blocks;
      int64_t block_id = tmp % num_blocks;
      ...
      int64_t split_size = div_up(seq_len_kv, kv_splits);
      int64_t kv_start = kv_id * split_size;
      int64_t kv_end = std::min(kv_start + split_size, seq_len_kv);

      int64_t h_start = head_kv_id * num_groups + block_id * block_size_h;
      int64_t h_end = std::min(h_start + block_size_h, head_kv_id * num_groups + num_groups);
      int64_t h_size = h_end - h_start;
      if (h_size <= 0) {
        continue;
      }
      ...
      for (int64_t n = kv_start; n < kv_end; n += BLOCK_N) {
        ...
        pack_vnni_value<scalar_t, index_t>(
            Btmp1, v_buffer + head_kv_id * v_strideH, indices, n_size, head_size_v, v_strideN, head_size_v);
...
void decode_attention_cpu_opt(
    ...
    double logit_cap) {
  RECORD_FUNCTION(
      "sgl-kernel::decode_attention_cpu_opt",
  ...
  TORCH_CHECK(
      ...
      "decode_opt: expect req_to_token to be int32 or int64, got ",
      index_dtype);
  TORCH_CHECK(
      seq_lens.scalar_type() == at::kLong,
      "decode_opt: expect req_lens to be int64, got ",
      seq_lens.scalar_type());
  TORCH_CHECK(
      req_pool_indices.scalar_type() == at::kLong,
      "decode_opt: expect req_pool_indices to be int64, got ",
  ...
  const int64_t configured_threads = at::get_num_threads();
  int64_t num_threads = configured_threads;
  TORCH_CHECK(num_threads > 0, "decode_opt: expect num_threads > 0, got ", num_threads);
  TORCH_CHECK(
      attn_logits_splits == configured_threads,
      "decode_opt: pooled attn_logits.shape[2] must equal configured threads, got attn_logits.shape[2]=",
      attn_logits_splits,
      ", configured_threads=",
      configured_threads);

  // Batch-1 optimization: cap thread set by sequence-length coverage.
  if (num_seqs == 1) {
    constexpr int64_t BLOCK_N_OPT = 128;
    const int64_t seq_len = seq_lens.data_ptr<int64_t>()[0];
    const int64_t seq_cap = std::max<int64_t>(1, seq_len / BLOCK_N_OPT);
    num_threads = std::min<int64_t>(num_threads, seq_cap);
  }

  // Ensure split capacity is satisfiable for the selected thread set.
  if (is_mla) {
    num_threads = std::min<int64_t>(num_threads, attn_logits_splits);
    TORCH_CHECK(num_threads >= 1, "decode_opt MLA: insufficient thread budget after cap.");
  } else {
    const int64_t denom = num_heads_kv;
    num_threads = std::min<int64_t>(num_threads, attn_logits_splits * denom);
    TORCH_CHECK(num_threads >= denom, "decode_opt packed GQA: insufficient thread budget after cap.");
    num_threads = (num_threads / denom) * denom;
  }
  TORCH_CHECK(num_threads > 0, "decode_opt: derived num_threads must be > 0.");

  int64_t total_kv_splits_budget = 0;
  if (is_mla) {
    total_kv_splits_budget = num_threads;
  } else {
    const int64_t denom = num_heads_kv;
    TORCH_CHECK(
        num_threads % denom == 0,
        "decode_opt packed GQA: expect num_threads % num_heads_kv == 0, got num_threads=",
        num_threads,
        ", num_heads_kv=",
        num_heads_kv);
    total_kv_splits_budget = num_threads / denom;
  }
  TORCH_CHECK(total_kv_splits_budget > 0, "decode_opt: derived total split budget must be > 0.");

  std::vector<int64_t> kv_splits_per_seq =
      allocate_kv_splits(seq_lens.data_ptr<int64_t>(), num_seqs, total_kv_splits_budget);
  std::vector<int64_t> seq_split_prefix(num_seqs + 1, 0);
  for (int64_t i = 0; i < num_seqs; ++i) {
    seq_split_prefix[i + 1] = seq_split_prefix[i] + kv_splits_per_seq[i];
  }
  TORCH_CHECK(
      seq_split_prefix.back() <= attn_logits_splits,
      "decode_opt: pooled split usage exceeds attn_logits capacity, used=",
      seq_split_prefix.back(),
      ", capacity=",
      attn_logits_splits);
  int64_t max_kv_splits_per_seq = *std::max_element(kv_splits_per_seq.begin(), kv_splits_per_seq.end());
  TORCH_CHECK(
      attn_logits_splits >= max_kv_splits_per_seq,
      "decode_opt: attn_logits.shape[2] must be >= max per-seq split (",
      max_kv_splits_per_seq,
      "), got ",
      attn_logits_splits);
  ...
  if (is_mla) {
    TORCH_CHECK(block_n == BLOCK_N_MLA, "decode_opt MLA: unsupported BLOCK_N ", block_n);
  }

  int64_t num_blocks = head_block_num;
  int64_t block_size_h = 0;
  if (is_mla) {
    block_size_h = div_up(num_heads, num_blocks);
  } else {
    int64_t num_groups = num_heads / num_heads_kv;
    block_size_h = div_up(num_groups, num_blocks);
  }

  int64_t size_per_thread = (is_mla ? BLOCK_N_MLA : BLOCK_N_GQA_PACKED) * (head_size + head_size_v);
  auto buffer = at::empty({configured_threads, size_per_thread}, k_buffer.options());

  int64_t per_seq_task_mul = is_mla ? num_blocks : (num_heads_kv * num_blocks);
  std::vector<int64_t> seq_task_prefix = build_seq_task_prefix(kv_splits_per_seq, per_seq_task_mul);

  AT_DISPATCH_REDUCED_FLOATING_TYPES(query.scalar_type(), "decode_attention_adaptive_kernel", [&] {
    AT_DISPATCH_INDEX_TYPES(index_dtype, "decode_attention_adaptive_indices", [&] {
      ...
      if (is_mla) {
        decode_attention_mla_adaptive_kernel_impl<scalar_t, index_t, BLOCK_N_MLA>(
            ...
            kv_splits_per_seq.data(),
            seq_task_prefix.data(),
            seq_split_prefix.data(),
        ...
        decode_attention_grouped_packed_adaptive_kernel_impl<scalar_t, index_t, BLOCK_N_GQA_PACKED>(
            output.data_ptr<scalar_t>(),
            attn_logits.data_ptr<float>(),
            query.data_ptr<scalar_t>(),
            (const scalar_t*)k_buffer_data,
