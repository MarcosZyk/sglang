...
TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {
  ...
  m.def(
      "decode_attention_cpu_tuned(Tensor query, Tensor k_cache, Tensor v_cahce, Tensor(a!) output, Tensor key, Tensor value, "
      "Tensor loc, Tensor attn_logits, Tensor req_to_token, Tensor req_pool_indices, Tensor seq_lens, float sm_scale, "
      "float logit_cap, int head_block_num, int num_kv_splits) -> ()");
  m.impl("decode_attention_cpu_tuned", torch::kCPU, &decode_attention_cpu_tuned);
  m.def(
      "decode_attention_cpu_tdm(Tensor query, Tensor k_cache, Tensor v_cahce, Tensor(a!) output, Tensor key, Tensor value, "
      "Tensor loc, Tensor attn_logits, Tensor req_to_token, Tensor req_pool_indices, Tensor seq_lens, float sm_scale, "
      "float logit_cap, int head_block_num) -> ()");
  m.impl("decode_attention_cpu_tdm", torch::kCPU, &decode_attention_cpu_tdm);
  m.def(
      "decode_attention_cpu_adaptive(Tensor query, Tensor k_cache, Tensor v_cahce, Tensor(a!) output, Tensor key, Tensor value, "
      "Tensor loc, Tensor attn_logits, Tensor req_to_token, Tensor req_pool_indices, Tensor seq_lens, float sm_scale, "
      "float logit_cap, int head_block_num) -> ()");
  m.impl("decode_attention_cpu_adaptive", torch::kCPU, &decode_attention_cpu_adaptive);
  m.def(
      "decode_attention_cpu_opt(Tensor query, Tensor k_cache, Tensor v_cahce, Tensor(a!) output, Tensor key, Tensor value, "
      "Tensor loc, Tensor attn_logits, Tensor req_to_token, Tensor req_pool_indices, Tensor seq_lens, float sm_scale, "
      "float logit_cap) -> ()");
  m.impl("decode_attention_cpu_opt", torch::kCPU, &decode_attention_cpu_opt);
  m.def("decode_timer_start(Tensor placeholder, bool reset=True) -> ()");
  m.impl("decode_timer_start", torch::kCPU, &decode_timer_start);
  m.def("decode_timer_stop_and_print(Tensor placeholder) -> ()");
  m.impl("decode_timer_stop_and_print", torch::kCPU, &decode_timer_stop_and_print);
